...
class PatchEmbedding(nn.Module):
	...
	def forward(self, x):
		x = self.cnn(x) if self.early_cnn else x #(n, 3, 32, 32) doesn't change the size ofthe image
...
class ViT2(nn.Module):
	'''
	   It is the same as the previous class. The only diffence is that it makes basic blocks differently.

	'''

	def __init__(self, img_size , patch_size = 16, in_channels = 3, n_classes = 1000, 
				 embed_dim = 784, layers = 6, n_heads = 8, p_drop = 0.1):
		...
		self.blocks = self._make_block(Block, num_blocks = layers, n_heads = n_heads, embed_dim = embed_dim)
	...
	def forward(self, x):
		...
		x = torch.cat([cls_token, x], dim = 1) #(n_samples, 1+n_patches, embed_dim) First patch is the cls token
		x = x + self.pos_embed	
		x = self.pos_drop(x)
		x = self.blocks(x)
		x = self.norm(x) 

		output = x[:, 0]  #It is actually x[:, 0, :]
		x = self.head(output)
	...
	def _make_block(self, basicblock, num_blocks, n_heads, embed_dim):
		layers = []

		for i in range(num_blocks):
			layers.append(basicblock(dim = embed_dim , n_heads = n_heads))
...
#TEST model 1
model = ViT(img_size = 32, patch_size = 8, in_channels = 3, n_classes = 10, embed_dim = 768, layers = 8, n_heads = 8)
out = model(x)
print(out.shape)
